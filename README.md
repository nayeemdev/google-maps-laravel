...
```php
        Nayeemdev\GoogleMaps\GMapsServiceProvider::class,
```

And finally add in the alias section `config/app.php`
```php
        'GMaps' => Nayeemdev\GoogleMaps\Facades\GMapsFacade::class,
```

Now publish configuration file
```php
    php artisan vendor:publish --provider="Nayeemdev\GoogleMaps\GMapsServiceProvider"
```

##Usage
Example files are under Nayeemdev/GoogleMaps/Example:
